...
§
GitHub state backup: Tutu wants significant changes to Hermes state committed/pushed to backup repo AND notified on Telegram. Mechanism: on-session-finalize 'hermes-backup' plugin -> hermes_backup.py at C:\Users\bohen\hermes-backup\ -> github.com/NanaTutu/my-hermes-agent-backup.git, then sends a Telegram update via `hermes send --to telegram`. On significant change ensure a commit+push happens. .env/auth.json/state.db deliberately excluded (secrets stay local).
§
